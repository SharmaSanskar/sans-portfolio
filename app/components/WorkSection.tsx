...
const WorkSection = () => {
  ...
  return (
    <section
      className="relative min-h-[60vh] p-4 md:p-8 bg-bgDark-light dark:bg-bgDark"
      ref={sectionRef}
      id="work"
    >
      {/* Desktop title */}
      <div className="hidden md:flex absolute -right-12 top-1/2 -translate-y-1/2 items-center">
        <div className="transform rotate-90 text-4xl tracking-widest font-bold flex items-center gap-3">
          <h1 className="text-fontPrimary-light dark:text-fontPrimary">WORK</h1>
      ...
      <div className="relative pt-8 md:pt-16 w-full md:w-3/4 mx-auto mt-4 md:mt-12">
        {/* Timeline line */}
        <motion.div
          className="absolute top-24 left-0 right-0 h-1 rounded-full bg-accentPurple2-light dark:bg-accentPurple2"
          initial={{ scaleX: 0 }}
        ...
        {/* Experience items */}
        <div className="relative flex flex-col md:flex-row md:justify-around gap-8 md:gap-4 text-fontPrimary-light dark:text-fontPrimary">
          {experiences.map((exp, index) => (
            <div key={index} className="w-full md:w-[40%] relative">
              {/* Date label */}
              <motion.div
                className="text-sm text-fontSecondary-light dark:text-fontSecondary mb-2 text-center md:absolute md:-top-1 md:left-[50%] md:transform md:-translate-x-1/2"
              ...
              {/* Experience box */}
              <motion.div
                className="bg-accentPurple1-light dark:bg-accentPurple1 px-6 md:px-12 py-6 md:py-8 rounded-xl mt-2 md:mt-12"
                initial={{ opacity: 0, y: -40 }}
                animate={{ opacity: isInView ? 1 : 0, y: isInView ? 0 : -40 }}
                transition={{
                  duration: 0.8,
